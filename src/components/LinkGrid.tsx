...
import type { LinkGroup, Theme, Link } from '../constants';
...
export const LinkGrid: React.FC<LinkGridProps> = ({
  ...
  const isLight = theme === 'light';
  const emptyClass = isLight ? 'text-gray-500/50 border-gray-500/10' : 'text-white/20 border-white/5';
            ...
            {!isCollapsed && (
              <>
                {group.links.length > 0 ? (
                  ...
                  <div className={`text-center py-6 text-xs italic border-2 border-dashed rounded-xl ${emptyClass}`}>
                    Empty Group
                  </div>
                )}
              </>
