        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
            {candidates.map(b => (
                <div key={b.id} onClick={() => onToggle(b.id)} className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors ${selectedIds.has(b.id)
                    ...
                    <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${selectedIds.has(b.id) ? 'bg-blue-600 border-blue-600' : 'border-gray-400'}`}>
