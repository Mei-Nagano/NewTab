...
export default function App() {
  ...
  return (
    ...
    >
        ...
        <div
          ...
          style={{
            backgroundColor: backgroundImage
              ? `rgba(0, 0, 0, ${(settings.darkMaskOpacity ?? 40) / 100})`
              : '#0f1115'
          }}
        />
      )}
      ...
      {/* 可滚动内容容器 */}
      <div className={`absolute inset-0 z-10 ${settings.linkDisplayMode === 'pagination' ? 'overflow-hidden' : 'overflow-y-auto'} custom-scrollbar`}>
        <div className="min-h-full w-full flex flex-col items-center py-12">
          ...
          <div className="flex flex-col items-center gap-10 w-full max-w-6xl px-4 my-auto animate-fade-in">
            {(!settings.hideOptions?.hideClock || !settings.hideOptions?.hideDate) && (
              <div className="flex-shrink-0 flex flex-col items-center gap-8 w-full">
                <Clock
                  showTime={!settings.hideOptions?.hideClock}
      ...
      {!settings.hideOptions?.hideButtons && (
        <button
          ...
          className={`absolute top-6 right-6 z-20 p-3 rounded-full backdrop-blur-xl transition-all duration-500 hover:scale-110 hover:rotate-180 border shadow-2xl group ${isLight ? 'bg-white/70 hover:bg-white/90 text-orange-600 border-orange-200/50 shadow-orange-500/20' : 'bg-gray-900/70 hover:bg-gray-900/90 text-yellow-400 hover:text-yellow-300 border-yellow-500/30 shadow-yellow-500/20'}`}
          aria-label="切换主题"
