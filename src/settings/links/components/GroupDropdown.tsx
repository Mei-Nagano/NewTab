...
export const GroupDropdown: React.FC<GroupDropdownProps> = ({
  ...
  return (
    <div className="relative flex-1">
      <button
        onClick={onToggle}
        className={`w-full px-4 py-3 rounded-2xl border flex items-center justify-between transition-all duration-300 ${isLight
      ...
      >
        <div className="flex items-center gap-2.5 text-left">
          <div className={`p-1.5 rounded-lg ${isLight ? 'bg-blue-50 text-blue-500' : 'bg-blue-500/10 text-blue-400'}`}>
            <Layers size={16} />
          </div>
          <div>
            <span className={`text-[9px] font-black uppercase tracking-widest ${isLight ? 'text-gray-400' : 'text-gray-500'}`}>选择分组</span>
            <div className="text-sm font-black truncate max-w-[120px] md:max-w-none">{activeGroup?.title || ALL_GROUP_TITLE}</div>
          </div>
        </div>
        <ChevronDown size={18} className={`transition-transform duration-500 ${isOpen ? 'rotate-180 text-blue-500' : 'text-gray-400'}`} />
      </button>

      {isOpen && (
        <div className={`absolute top-full left-0 right-0 mt-2 z-[60] border backdrop-blur-xl animate-scale-in rounded-2xl shadow-2xl overflow-hidden ${isLight ? 'bg-white/95 border-gray-100' : 'bg-gray-900/95 border-white/10'
          }`}>
          <div className="p-1.5 max-h-64 overflow-y-auto custom-scrollbar space-y-0.5">
            <button
              onClick={() => onSelect(ALL_GROUP_ID)}
              className={`w-full text-left px-3.5 py-2.5 text-sm rounded-xl flex items-center justify-between transition-all duration-300 ${isAllGroupSelected
            ...
            <div className={`h-px mx-2 my-0.5 ${isLight ? 'bg-gray-100' : 'bg-white/5'}`} />
            {groups.map((group) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3.5 py-2.5 text-sm rounded-xl flex items-center justify-between transition-all duration-300 ${isSelected
                      ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/20'
                      : isLight ? 'text-gray-700 hover:bg-gray-100/50' : 'text-gray-300 hover:bg-white/5'
                    }`}
                >
